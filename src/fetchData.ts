...
const fetchDOM = async (url: string) => {
	...
	const formatData = (
		...
		const paragraphText =
			typeof section !== "undefined" ? section.find("p").text() : "";
		if (!headerText.match(/\.prototype\./))
			return {
				index: "",
				header: "",
				name: "",
